...
bool Parameters::arrange() {
	...
	if (num_nodes<=0 || average_k<=0 || max_degree<=0 || mixing_parameter<0 || mixing_heterogeneity<0 || (nmax<=0 && nmax!=unlikely) || (nmin<=0 && nmin!=unlikely) ) {
...
void statement() {
	...
	cout<<"-mu\t\t[mixing parameter]"<<endl;	
        cout<<"-delta\t\t[mixing heterogeneity parameter]"<<endl;	
