...
void rebuild_internal_degree_heterogeneity(double mixing_parameter, double mixing_heterogeneity, deque<deque<int> >  & member_matrix,  deque<int> & degree_seq, deque<int> &internal_degree_seq, deque<int> &member_numbers){
    ...
    for (i=0; i<degree_seq.size(); i++){
        ...
        internal_degree_seq[i] = degree_seq[i] - (int)round(average_mu_seq[i]*degree_seq[i]);
    }
...
bool excess, bool defect,  deque<int> & degree_seq, deque<int> &num_seq, deque<int> &internal_degree_seq, bool fixed_range, int nmin, int nmax, double tau2) {
	...
	*/
	...
	//cout<<"internal_degree_seq before the variation"<<endl;
	//prints(internal_degree_seq);
        ...
        if (mixing_heterogeneity>0)
            rebuild_internal_degree_heterogeneity(mixing_parameter, mixing_heterogeneity, member_matrix, degree_seq, internal_degree_seq, member_numbers);
